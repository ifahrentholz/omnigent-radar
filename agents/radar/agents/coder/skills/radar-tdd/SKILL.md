...
- **Unfalsified guard** — a test, lint rule, or CI check that asserts a _rule_ (a purity guard, an architectural boundary, a pinned invariant, a policy) and has never been observed failing. The tell: nobody can say what would break it. Such a check is green because it checks the wrong thing — the wrong path, a name instead of a behaviour, a file that moved — and that green is indistinguishable from real protection. A guard nobody has seen red is not a guard, it is a comment.
